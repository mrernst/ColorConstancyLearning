...
def main(args):
	...
	for scene_id in range(args.n_cubes):
		...
		for light in range(N_LIGHTS):
			...
			while current_frame <= N_FRAMES+OFFSET:
				...
				wattage = (np.random.random()*(1-MIN_WATT/MAX_WATT) + MIN_WATT/MAX_WATT) * MAX_WATT # 300 -1000 Watt random luminosity
				...
				if args.plot_temp_overview:
					...
					ax[light].set_xlim([OFFSET,N_FRAMES])
		...
		#print(light_colors[155])
		#print(dict_of_lights[0][-1][155], dict_of_lights[1][-1][155], dict_of_lights[2][-1][155], dict_of_lights[3][-1][155], dict_of_lights[4][-1][155], dict_of_lights[5][-1][155], dict_of_lights[6][-1][155], dict_of_lights[7][-1][155])
	...
	#for scene_id in range(1): #range(args.n_cubes):
	for scene_id in range(args.n_cubes):
...
if __name__ == '__main__':
	...
	parser.add_argument('--plot_temp_overview', type=bool, default=False, help="Plot an overview over the scene")
	...
	parser.add_argument('--max_periodicity', type=int, default="15", help="maximum frames for lights cycle")
