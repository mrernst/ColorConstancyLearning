...
from utils.datasets import C3Dataset
...
from utils.evaluation import get_representations, lls_fit, lls_eval, supervised_eval, knn_eval, wcss_bcss, get_pacmap, get_neighbor_similarity, train_linear_classifier
...
def train():
    ...
    dataset_train_eval = DATASETS[args.dataset]['class'](
        ...
        split=args.eval_train_split if args.eval_train_split else args.train_split,
    ...
    optimizer = torch.optim.AdamW(net.parameters(), lr=args.lrate, weight_decay=args.weight_decay)
    ...
    if args.main_loss == 'supervised':
        ...
        features_train_eval, labels_train_eval = get_representations(net, dataloader_train_eval, device=device)
        features_test, labels_test = get_representations(net, dataloader_test, device=device)
        ...
        pacmap_plot = get_pacmap(features_test, labels_test, 0, dataset_test.n_classes, dataset_test.labels)
    ...
    for epoch in epoch_loop:
        ...
        for (x, x_pair), labels in dataloader_train:
            ...
            if args.reg_loss:
                if args.reg_loss == 'Decorrelation':
                    representation = representation.split(projection.shape[0]//2)[0].T
                    loss += args.decorr_weight * REG_LOSS[args.reg_loss](representation, pair)
                else:
        ...
        if (epoch+1) % args.test_every == 0 or (epoch+1) == args.n_epochs:
            ...
            if args.main_loss == 'supervised':
                ...
                similarity_plot = get_neighbor_similarity(features_test, labels_test, epoch + 1)
