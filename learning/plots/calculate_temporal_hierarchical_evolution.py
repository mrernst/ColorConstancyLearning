...
object_accuracy_array = np.zeros([5,5,6])
...
# define an array that stores the accuracies
lighting_accuracy_array = np.zeros([5,5,6])
...
for i,epoch in enumerate([1,25,50,75,100]):
	# load the data into the model
	# refresh the layers
	model,l5,l4,l3,l2,l1,l0 = load_model_and_layers(model_path=args.exp_dir + '/models/' + f'backbone_epoch_{epoch}.pt')    
	for j, (layer, out_dim) in enumerate(zip([l0, l1, l2, l3, l4, l5],[3072, 1176, 400, 120, 84, 128])):
		if out_dim == 3072:
			pass
		else:
