...
db_list = [temp_list, rand_list, jitter_list]
...
for s in range(ITERATIONS):
	for d in range(len(DBARRAY)):
		for l in range(len(CONDARRAY)):
			data[s, d, l] = np.load(f'./save/{db_list[d][s]}/object_classifier/{layer_list[l]}_accloss_random.npy')[-1]
...
data[:,:,0] = 1. - np.random.random([5,3])

# raw pixels in the rand dataset
data[0,0,0] = 0.4815
data[1,0,0] = 0.5115
data[2,0,0] = 0.4941
data[3,0,0] = 0.5068
...
# raw pixels in the temp dataset
data[0,1,0] = 0.5949
data[1,1,0] = 0.5922
data[2,1,0] = 0.5887
data[3,1,0] = 0.5902
...
for k in range(1):
	print(k)
	if COLORSCHEME=="monochrome":
		rects.append(ax.bar(0+k*bar_width*3, means[k,0], bar_width*3,
		alpha=opacity, **next(styles),
		yerr=stderror[k,0], error_kw=error_config, linewidth=0, edgecolor=lighten_color(current_palette[(k)])))
	else:
		rects.append(ax.bar(0+k*bar_width*3, means[k,0], bar_width*3,
		alpha=opacity,
		yerr=stderror[k,0], error_kw=error_config, linewidth=0, edgecolor=lighten_color(current_palette[(k+8)]), hatch="/////", color=current_palette[(k+8)]))
...
ax.text(-0.18, 1.05, ['A','B','C','D','E','F'][2], transform=ax.transAxes, size=21, weight='bold')
fig.subplots_adjust(bottom=.18)
fig.subplots_adjust(left=.14)
ax.legend(frameon=True, facecolor='white', edgecolor='white', framealpha=1.0, fontsize=10, loc='upper left')
ax.set_ylim(0,1)
plt.show()
